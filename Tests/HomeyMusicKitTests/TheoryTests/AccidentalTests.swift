...
final class AccidentalTests {
    ...
    // Test displayCases static computed property
    @Test func testDisplayCases() async throws {
        let display = Accidental.displayCases
        #expect(display.count == 2)
        #expect(display.contains(.flat))
        #expect(display.contains(.sharp))
        #expect(display.contains(.none) == false)
    }
}
